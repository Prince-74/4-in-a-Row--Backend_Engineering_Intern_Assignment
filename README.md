...
| --- | --- |
| **Backend** | Node.js, Express, Socket.IO, PostgreSQL, Prisma ORM, Kafka |
| **Frontend** | React (Vite), socket.io-client, CSS |
| **Infra / Tools** | Docker (Local Kafka), Render (Backend), Vercel (Frontend) |
...
-   **Producer**: `backend/src/kafka/producer.js` emits events (`GAME_STARTED`, `GAME_COMPLETED`, `GAME_FORFEITED`).
...
| `ANALYTICS_PERSIST_METRICS` | Whether to save metrics to DB |
...
-   Set `CLIENT_ORIGIN=https://YOUR_FRONTEND_URL` (Ensure `https://` is included).
    

### Frontend (Vercel)

-   Set `VITE_BACKEND_URL=https://YOUR_RENDER_BACKEND_URL`.
...
-   **CORS Errors**: Ensure `CLIENT_ORIGIN` matches your frontend URL exactly (including `https://`).
